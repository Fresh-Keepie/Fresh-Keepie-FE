...
const Signup = () => {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      const response = await axios.post('http://fresh-keepie-env.eba-kygb2spd.ap-northeast-2.elasticbeanstalk.com/user/signup', {
        userId: identifier,
        password,
      ...
      if (error.response) {
        ...
        if (responseError === 'Invalid form data') {
          ...
          } else if (responseError.includes('Nickname already exists')) {
