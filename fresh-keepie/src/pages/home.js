...
const updateProduct = (updatedProduct) => {
 ...
 console.log(`Product: ${product.productName}, Expiry Date: ${product.expiryDate}, Days left: ${diff}`);
      if (diff <= 1) {
        dday1.push(product);
      } else if (diff <= 6) {
        dday7.push(product);
    ...
    <Layout>
      ...
      <ContentLayout>
        <ItemLayout>
          ...
          <Calendar onDateClick={handleDateClick} products={products}/>
        </ItemLayout>
        <DateLayout>
          <DdayImgContainer>
            <ProductList>
              {dday1Products.map((product, index) => (
                <ProductItem key={index} onClick={() => openEditModal(product)}>
